...
export class COFormsExperimentComponent {
  ...
  form = new FormGroup<ClinicalOrderForm>({
    ...
    scheduling: new FormGroup<SchedulingForm>(
      ...
      {
        validators: [schedulingDateRangeValidator()],
      },
    ),
  ...
  constructor() {
    this.updatePhysicianState(this.orderDetails.controls.priority.value);
  ...
  createMedicationForm(medication?: Medication): FormGroup<MedicationForm> {
  ...
  addMetadataField(key: string, value: string): void {
    this.metadata.addControl(key, new FormControl(value));
  }
  ...
  addCustomField(key: string, value: string): void {
    this.customFields.addControl(key, new FormControl(value));
  }
  ...
  updatePhysicianState(priority: string | null): void {
    ...
    if (priority === 'STAT') {
      physician.setValue('Emergency Team', {
        emitEvent: false,
      });
    }
    ...
    this.mockService
      ...
      .subscribe((medications) => {
        ...
        medications.forEach((medication) => {
          this.medications.push(this.createMedicationForm(medication));
        });
      });
  }
    ...
    this.mockService
      ...
      .subscribe((fields) => {
        ...
        fields.forEach((field) => {
          this.metadata.addControl(field.key, new FormControl(field.value));
        });
      });
  }
}
